"""L-SHADE adaptive differential evolution implementation used in the experiments."""
...
class LSHADEOptimizer:
    """L-SHADE with success-history memories and linear population reduction."""
    ...
    def optimize(self) -> AlgorithmResult:
        initial_size = max(6, int(self.config.population))
        min_size = 4
        ...
        for _ in range(self.config.max_cycles):
            ...
            p_count = max(2, int(np.ceil(0.11 * current_size)))
            ...
            for i in range(current_size):
                ...
                cr = float(np.clip(self.rng.normal(memory_cr[mem], 0.1), 0.0, 1.0))
    ...
    def _target_population_size(self, initial_size: int, min_size: int, max_evaluations: int) -> int:
        ...
        target = round(initial_size + ratio * (min_size - initial_size))
        return max(min_size, int(target))

    def _sample_f(self, mean: float) -> float:
        for _ in range(32):
            value = mean + 0.1 * self.rng.standard_cauchy()
            if value > 0.0:
